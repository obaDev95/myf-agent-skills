...
Conventions for Vitest/Cypress specs around migrated `*HtmlTable.vue` and related views in this repo.

## Mandatory rules

1. **BDD-style `it` titles** where practical: *when …, then …* (or equivalent clear behavior wording).
...
When a bug is “wrong thing visible on small viewport,” add or tighten tests that **toggle `appStore.isSmallScreen`** (or equivalent) and assert the **same user-visible fields** the legacy table showed—then fold that into the skill’s [Mobile conditional render translation](../SKILL.md#mobile-conditional-render-translation-systematic-audit) checklist.
