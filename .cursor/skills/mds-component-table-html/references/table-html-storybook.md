# Table HTML — Storybook Notes (Playwright snapshot: 31 Jan 2026)

Source: `https://mds.maersk.io/?path=/story/components-table-html--documentation` captured via Playwright headless Chromium (`tools/playwright-scraper/scrape-table-html.js`). Additional class metadata pulled from `@maersk-global/mds-foundations/scss/_table.scss` (ships with MDS v2.170.x).
...
## Base markup (from Playwright capture)
```html
<body class="mds">
