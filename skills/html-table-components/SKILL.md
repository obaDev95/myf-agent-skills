...
- **Colspans are computed, not hard-coded.** `desktopColspan` = count of visible columns + (selection column present ? 1 : 0). `mobileExpandedColspan` comes from `useInvoiceHtmlTableSelectionAndRows`.
