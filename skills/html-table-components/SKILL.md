...
2. **No `:deep()` on native table markup in this SFC.** `<table>`, `<thead>`, `<tbody>`, `<th>`, `<td>`, `.mds-table__column--row-expander`, `.mds-table__column--row-selector`, `.mds-table__expanded-row` all live inside the same SFC as the styles; Vue's scoped transform already applies. `:deep()` is reserved for piercing into **another** component's template (rare here).
   ...
   **Why this rule exists.** Legacy `<mc-table>` rendered its `<table>` inside a shadow DOM, so scoped styles had to use `:deep(...)` to reach the header/cell markup inside the web component. After migration, the table is a normal `<table>` in the SFC's own template — scoped CSS applies directly. Shipped invoice-tab `*HtmlTable.vue` files still carry blocks like `:deep(.mds-table thead th) { padding-inline: 8px; }` left over from the legacy pattern; these render identically with or without `:deep`, but the `:deep` wrapper signals shadow-DOM piercing that no longer exists and blocks the "scoped CSS is local" invariant. Remove the `:deep` wrapper when you touch the file, or schedule a cleanup pass across the four invoice tables. Estatement ships without `:deep` and is the reference shape.
