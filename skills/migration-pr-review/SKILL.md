...
- [ ] Lifecycle resets (`resetDivisionFilter`, `REMOVE_CANCELLED_INVOICES`, etc.) fire at a semantically equivalent point. `onMounted` / `onBeforeMount` and `onUnmounted` / `onBeforeUnmount` are interchangeable for store resets that have no ordering dependency on child DOM; the per-tab section in **[../invoice-html-table-migration/SKILL.md](../invoice-html-table-migration/SKILL.md)** lists the expected hook per variant. A shipped table using `onUnmounted` where the legacy used `onBeforeUnmount` is not a finding unless the reset has an ordering dependency — call out the difference in the PR description but do not Block.
...
- [ ] `<style scoped>` does not duplicate a block (row-expander, sort-button, expanded-cell, desktop-notification-row, row-state) that a sibling `*HtmlTable.vue` already ships. **Classification:** duplication **introduced** by this PR is a **Block** — it must either be extracted to the shared partial in the same commit or rewritten to consume the partial. Duplication that **predates** the `shared-table-scss-refactor` skill (the four shipped invoice tables each carry ~100 lines of shared chrome in their scoped styles) is **Follow-up** debt and must not Block an otherwise-correct PR unless the PR already touches the duplicated block. When touching one of the four invoice `*HtmlTable.vue` files for any reason, extract the touched block to `invoice-table.scss` and remove it from siblings in the same PR. See **[../shared-table-scss-refactor/SKILL.md](../shared-table-scss-refactor/SKILL.md)**.
...
- [ ] Feature groups match the canonical list: Sorting, Selection, Pagination, Page size, Rendering, Mobile expanded parity, plus the variant-specific groups (Cancelled rows for Paid, API sort for Disputed, Virtualization for Open). **Classification:** when a PR ships a new `*HtmlTable.spec.ts`, non-canonical describe names are a **Block**; when a PR touches a file whose describe names were never refreshed after the original migration (shipped specs with `describe("snapshots")`, `describe("<Name>HtmlTable methods")`, etc.), treat the rename as a **Follow-up** and open a ticket with a named owner — do not block an otherwise-correct PR on pre-existing describe debt. Rewrite the describe structure in the same PR when the spec is already being edited for another reason.
